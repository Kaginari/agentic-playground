...
installed once (`~/.claude/commands/isekai.md`) and the same across every world. What
...
3. Create `.isekai/` and `.isekai/{elf,orc,slime,tmp,instruments}/`, each with an empty
   `.gitkeep`. Copy the race portraits from `~/.claude/isekai/portraits/*.png` into
   `.isekai/portraits/` (skip any already there). If the source folder is missing or empty,
   tell the user instead of silently skipping — the doc templates below hard-link to these
   images. Every creature doc starts with its race portrait:
