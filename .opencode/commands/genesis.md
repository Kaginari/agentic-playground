...
Where `/isekai` founds the world, `/genesis` looks at the code the world actually contains and
decides, itself, which Elves, Orcs and Slimes that code needs — then births them.
...
- This command works identically whether launched from OpenCode or Claude Code — `.isekai/` is
  ecosystem-neutral.
